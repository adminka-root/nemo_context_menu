...
user_dialog () { # типичная функция диалога с пользователем
    ...
    if [[ ! -d "$way_rep" ]]; then echo "Ошибка: директории \"${OPTARG}\" не существует!"; exit_abnormal; fi
    if [[ "$(echo "$way_rep" | cut -c ${#way_rep})" != "/" ]]; then # если путь не заканчивается на слэш, добавить его
        way_rep+="/"
    fi
}
...
main_f() { # главная функция, инициализирует, все и вся
    ...
    way_rep="$HOME/.local/share/nemo_context_menu/" # где наш реп
